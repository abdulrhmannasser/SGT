...
<?php
    ...
    $rsltsearch=mysqli_query($cnsearch,"select m.id,m.upload_date,m.description,u.fullname,m.media_type,u.id  from media m join talented t on(t.id=m.talent_id) join users u on(u.id=m.talent_id) where m.state like 'accepted' and t.id=$tid order by  m.upload_date desc");
    $cntalented=mysqli_connect(Host,UN,PW,DBname);
    $rslttalented=mysqli_query($cntalented,"select u.fullname,t.mobile,t.address,t.img,t.email,t.bio from talented t  join users u on(u.id=t.id) where u.id=$tid");
    $talented=mysqli_fetch_array($rslttalented);
?>
    ...
    <div class="border border-secondary border-top-0" style="background-color:#2a81c9;color:#FFFFFF;padding-top:100px;padding-bottom:50px;">
        <div class="container text-center " style="">
            <div class="alert alert-info" dir="rtl" role="alert">
                <?php
                echo "اسم الموهبه : $talented[0] <br>" ;
                echo "جوال الموهبه : $talented[1] <br>" ;
                echo "عنوان الموهبه : $talented[2] <br>" ;
                echo "البريد الاليكتروني : $talented[4] <br>" ;
                echo "السيره الذاتيه : $talented[5] <br>" ;
                ...
                while ($arrsearch=mysqli_fetch_array($rsltsearch))
                {
                    ...
                    <div class="bg-white text-primary card ">
                        <a href="<?php if($arrsearch[4]=='video')echo "video.php?mid=$arrsearch[0]"; else echo "image.php?mid=$arrsearch[0]"; ?>">
                            ...
                            <div class="card-body">
                               <a href="viewtalented.php?tid=<?php echo $arrsearch[5]?>"> <h4 class="card-title"><?php echo $arrsearch[3] ?></h4></a>
